...
void Smerge_3way(SFile& file1, SFile& file2, SFile& file3, SFile& outFile)
{
    ...
    if (inIter1.it != end1)  
    {
        minheap.push(*inIter1.it);
    }
    if (inIter2.it != end2)  
    {
        minheap.push(*inIter2.it);
    }
    if (inIter3.it != end3)  
    {
        minheap.push(*inIter3.it);
    }
    ...
    do
    {
        ...
        {
            inbuf = inIter1.getNextBlock();
            if (inIter1.it != end1) minheap.push(*inIter1.it);
        }
        else if (inblk.fileNum == file2.fileNum)
        {
            inbuf = inIter2.getNextBlock();
            if (inIter2.it != end2) minheap.push(*inIter2.it);
        }
        else if (inblk.fileNum == file3.fileNum)
        {
            inbuf = inIter3.getNextBlock();
            if (inIter3.it != end3) minheap.push(*inIter3.it);
        }
        else
        {
            assert("not" == 0);
